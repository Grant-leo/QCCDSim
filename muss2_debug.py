...
class MUSSSchedule:
    def __init__(self, ir, gate_info=None, M=None, init_map=None,
                 serial_trap_ops=None, serial_comm=None, global_serial_lock=None):
        """
        支持两种调用方式：
        1) 传统方式：直接传入 ir, gate_info, M, init_map, serial_trap_ops, serial_comm, global_serial_lock
        2) 解析对象方式：第一个参数为 parse_obj，后续依次为 M, init_map, serial_trap_ops, serial_comm, global_serial_lock
        """
        # 判断第一个参数是否为解析对象（拥有 gate_graph 属性）
        if hasattr(ir, 'gate_graph') and hasattr(ir, 'all_gate_map'):
            parse_obj = ir
            self.ir = parse_obj.gate_graph
            self.gate_info = parse_obj.all_gate_map
            self.machine = gate_info  # 此时 gate_info 实际上是 machine
            self.init_map = M         # 此时 M 实际上是 init_map
            self.SerialTrapOps = init_map           # 此时 init_map 是 serial_trap_ops
            self.SerialCommunication = serial_trap_ops  # 此时 serial_trap_ops 是 serial_comm
            self.GlobalSerialLock = serial_comm          # 此时 serial_comm 是 global_serial_lock
            # 如果还有第七个参数，可忽略（因为已对齐）
        else:
            # 直接参数方式
            self.ir = ir
